...
class ModelEditor(tk.Toplevel):
    ...
    def group_selected(self):
        """Создание новой группы из выбранных элементов или групп"""
        if not self.current_selection:
            messagebox.showwarning("Группа", "Выберите хотя бы один элемент или группу")
            return
        ...
        # Определяем, есть ли родительская группа
        parent_group = None
        ...
        # Если все выбранные элементы из одной группы - это будет родительской группой
        if len(selected_group_names) == 1:
            parent_group = list(selected_group_names)[0]
        ...
        for ci in self.current_selection:
            # Если элемент был в родительской группе, удаляем его оттуда
            old_group = ci.layer.get("group")
            if old_group and old_group != name:
                for g in self.model.get("groups", []):
                    if g.get("name") == old_group and ci.layer.get("name") in g.get("children", []):
                        g["children"].remove(ci.layer.get("name"))
            
            # Назначаем элементу новую группу
            ci.layer["group"] = name
    ...
    def load_group_settings(self, group_name):
        """Загрузка настроек выбранной группы"""
        try:
            group = next((g for g in self.model.get("groups", []) if g.get("name") == group_name), None)
            if not group:
                return
            
            # Собираем все доступные слои и группы для выбора
            available_items = []
            available_groups = []
            
            # Слои внутри этой группы
            group_layers = [ci.layer.get("name") for ci in self.items if ci.layer.get("group") == group_name]
            available_items.extend(group_layers)
            
            # Дочерние группы
            child_groups = [g.get("name") for g in self.model.get("groups", []) if g.get("parent") == group_name]
            available_groups.extend(child_groups)
            
            # Загружаем логику группы
            saved_logic = group.get("logic", {})
            ...
            for state in ("silent", "whisper", "normal", "shout", "blink", "open"):
                ...
                # Добавляем слои
                for layer_name in available_items:
                    menu.add_command(
                        label=f"Слой: {layer_name}",
                        command=lambda val=layer_name, v=self.state_vars[state]: v.set(val)
                    )
                ...
                for grp_name in available_groups:
                    menu.add_command(
                        ...
                        command=lambda val=grp_name, v=self.state_vars[state]: v.set(val)
                    )
                
                # Загружаем сохраненное значение
                if state in saved_logic:
            ...
            blink_freq_value = group.get("blink_freq", 0.0)
            if blink_freq_value == "" or blink_freq_value is None:
                blink_freq_value = 0.0
            self.blink_freq.set(float(blink_freq_value))
    ...
    def _get_current_state_for_group(self, group_name, current_state):
        ...
        if group.get("random_effect", False) and self.random_effect_var.get():
            ...
            if now > self.group_random_timers.get(group_name, 0):
                # Собираем доступные варианты (слои и дочерние группы)
                available = []
                
                # Слои в группе
                group_layers = [ci.layer.get("name") for ci in self.items if ci.layer.get("group") == group_name]
                available.extend(group_layers)
                
                # Дочерние группы
                child_groups = [g.get("name") for g in self.model.get("groups", []) if g.get("parent") == group_name]
                available.extend(child_groups)
                
                # Исключаем слои для моргания и открытия глаз
                blink_layer = logic.get("blink", "")
                open_layer = logic.get("open", "")
                available = [item for item in available if item != blink_layer and item != open_layer]
    ...
    def _get_group_hierarchy(self):
        """Возвращает иерархию групп в виде словаря"""
        hierarchy = {}
        groups = self.model.get("groups", [])
    ...
    def _get_visible_items_for_state(self, current_state):
        ...
        def process_group(group_name):
            ...
            chosen = self._get_current_state_for_group(group_name, current_state)
            if not chosen:
                return
    ...
    def on_canvas_mouse_move(self, event):
        ...
        if self.selected_group:
            # Перемещаем все элементы в группе
            for s in self.items:
                if s.layer.get("group") == self.selected_group:
                    s.x += int(dx)
                    s.y += int(dy)
        else:
            # Перемещаем только выбранные элементы
